...
(function() {
  ...
  H5.DB.dado_prodes_consolidado.data = {
    ...
    populate: function(year, state, terra_indigena, uc_sustentavel, uc_integral, assentamento, floresta, dominio) {
      var self;
      if (state && year) {
        self = this.states[state];
        self[year] = {};
  ...
  for (i in _ref5) {
    properties = _ref5[i];
    H5.DB.dado_prodes_consolidado.data.populate(properties.ano, properties.uf, parseFloat(properties.terra_indigena), parseFloat(properties.unidades_de_conservacao_uso_sustentavel), parseFloat(properties.unidades_de_conservacao_protecao_integral), parseFloat(properties.assentamento), parseFloat(properties.floresta_publica), properties.dominio_estadual ? parseFloat(properties.dominio_estadual) : parseFloat(0));
  }
  ...
  chart10.drawChart = function() {
    ...
    if (rateSelected === '1') {
      ...
      $.each(rest.data[0], function(field, result) {
        ...
        data[1] = result ? parseFloat(result.toFixed(2)) : 0;
  ...
  chart11.drawChart = function() {
    ...
    createTable = function(states) {
      ...
      for (_l = 0, _len = period.length; _l < _len; _l++) {
        ...
        i = 1;
        _this.data.addRow(data);
        _this.data.addRow([null, null, null, null, null, null]);
      }
      return _this.data.removeRow(_this.data.getNumberOfRows() - 1);
    };
    this.createDataTable();
    this.data.addColumn("string", "Ano");
    this.data.addColumn("number", "Terra indígena em km²");
    this.data.addColumn("number", "Assentamento em km²");
    this.data.addColumn("number", "Terras Arrecadadas em km²");
  ...
  chart12.drawChart = function() {
    ...
    this.changeTitle("Distribuição de Detecções por Competência de Fiscalização [" + H5.Data.years[chart12.options.period] + "]");
    this._rightBtn.disabled = true;
    this._leftBtn.disabled = true;
    google.visualization.events.addListener(this.chart, "ready", function() {
      _this._rightBtn.disabled = _this.options.period < 1;
  ...
  chart13._leftBtn.onclick = function() {
    chart13.options.period++;
    return chart13.drawChart();
  };

  chart13._rightBtn.onclick = function() {
    chart13.options.period--;
    return chart13.drawChart();
  };
  ...
  chart13.drawChart = function() {
    ...
    this.changeTitle("Território de Competência Federal em " + years[this.options.period]);
    this._rightBtn.disabled = true;
    this._leftBtn.disabled = true;
    google.visualization.events.addListener(this.chart, "ready", function() {
      _this._rightBtn.disabled = _this.options.period < 1;
  ...
  chart14._leftBtn.onclick = function() {
    chart14.options.period++;
    return chart14.drawChart();
  };

  chart14._rightBtn.onclick = function() {
    chart14.options.period--;
    return chart14.drawChart();
  };
  ...
  chart14.drawChart = function() {
    var createTable, options,
      _this = this;
    createTable = function(states) {
      var data, estado, rate, state, sum, sumUC, year, _k, _l, _len, _len1, _ref7, _ref8, _results;
      data = [];
      sum = 0;
      sumUC = 0;
      years = H5.Data.years;
    ...
    this.changeTitle("Território de Competência Estadual em " + years[this.options.period]);
    this._rightBtn.disabled = true;
    this._leftBtn.disabled = true;
    google.visualization.events.addListener(this.chart, "ready", function() {
      _this._rightBtn.disabled = _this.options.period < 1;
