...
(function() {
  var chart1, chart10, chart11, chart12, chart13, chart2, chart3, chart4, chart5, chart6, chart7, chart8, chart9, chartAnnualEmbargo, chartDailyEmbargo, chartMonthlyEmbargo, chartStatesEmbargo, chartsDeter, chartsEmbargo, charts_content, i, knob1, knob2, knob3, lastSelectedRegion, properties, region, reloadChartsDeter, reloadChartsEmbargos, rest, selectMonths, selectType, selectYears, selectedYear, spark1, spark2, sparkAnnualEmbargo, sparkMonthlyEmbargo, sparkTVAAEmbargo, sparkTVMAEmbargo, sparkTVPAEmbargo, sparksDeter, sparksEmbargo, state, years, _i, _j, _k, _l, _len, _len1, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
  ...
  chart6.drawChart = function() {
    var data, name, options, state, sumDeter, sumProdes, _ref14;
  ...
  chart7.drawChart = function() {
    var data, estado, options, sumValues, _k, _ref14;
    sumValues = function(state, year) {
      var firstPeriod, key, reg, secondPeriod, sum, _ref14, _ref15;
      sum = 0;
      firstPeriod = new Date(year - 1, 7, 1);
  ...
  chart8.drawChart = function() {
    var data, daysInMonth, estado, firstPeriod, options, pieText, pieTooltip, secondPeriod, sumValues, _k, _ref14;
  ...
  $(charts_content._ratesSlct).on("change", function(event) {
    if (charts_content._ratesSlct.value === "0") {
      $("#ano_prodes").hide();
      return $("#periodo_deter").show();
    } else {
      $("#ano_prodes").show();
      return $("#periodo_deter").hide();
    }
  });

  $(charts_content._shapesSlct).on("change", function(event) {
    charts_content._domainSlct.options["0"].selected = true;
    if (charts_content._shapesSlct.value === "terra_indigena") {
      return charts_content._domainSlct.disabled = true;
    } else {
      return charts_content._domainSlct.disabled = false;
    }
  });

  $(charts_content._consultBtn).on("click", function(event) {
    chart10.drawChart();
    if (charts_content._ratesSlct.value === "1") {
      return chart13.drawChart();
    } else {
      return $("#chart13").hide();
    }
  });

  $.each(charts_content._stateGroup, function() {
    return $(this).on("click", function(event) {
      var _ref14;
      return charts_content._state = (_ref14 = $(this).children('span').html()) != null ? _ref14 : 'Brasil';
    });
  });

  chart10.drawChart = function() {
    var areaSelected, createTable, data, dateBegin, dateEnd, deter_area, deter_area_state, deter_field, deter_table, domainSelected, function_name, months, options, partialBegin, partialEnd, rateSelected, rates, shapes, state, stateData, timeBegin, timeBetween, timeEnd, year, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q, _r, _ref14, _ref15, _ref16, _ref17, _ref18, _ref19, _ref20, _ref21, _ref22, _s;
    createTable = (function(_this) {
      return function(state) {
        ...
        for (day = _k = 1; 1 <= daysInMonth ? _k <= daysInMonth : _k >= daysInMonth; day = 1 <= daysInMonth ? ++_k : --_k) {
          $.each(H5.DB.diary.data.states[state], function(key, reg) {
            var _ref14;
            if ((firstPeriod <= (_ref14 = reg.date) && _ref14 <= secondPeriod) && reg.day === day) {
              sum += reg.area;
              return false;
            }
          });
    ...
    if (charts_content._state !== 'Brasil') {
  ...
  chart11.drawChart = function() {
    ...
    createTable = (function(_this) {
      return function(states) {
        var data, estado, j, period, rate, state, sum, totalsum, year, _k, _l, _len, _len1, _len2, _m, _n, _ref14, _ref15, _ref16;
        sum = 0;
        totalsum = 0;
        data = [];
        i = 1;
        period = [];
        for (j = _k = 0, _ref14 = _this.options.period + 3; 0 <= _ref14 ? _k <= _ref14 : _k >= _ref14; j = 0 <= _ref14 ? ++_k : --_k) {
          period[j] = H5.Data.years[j];
        }
        for (_l = 0, _len = period.length; _l < _len; _l++) {
          year = period[_l];
          data[0] = year;
          _ref15 = ["terra_indigena", "assentamento", "floresta", "uc_integral", "uc_sustentavel"];
          for (_m = 0, _len1 = _ref15.length; _m < _len1; _m++) {
            rate = _ref15[_m];
            _ref16 = H5.Data.statesProdes;
            for (_n = 0, _len2 = _ref16.length; _n < _len2; _n++) {
              state = _ref16[_n];
    ...
    this.data[0].addColumn("number", "UC Integral em km²");
  ...
  chart12.drawChart = function() {
    var createTableCompetencias, createTableEstadual, createTableFederal, options, _k, _l, _ref14, _ref15, _results;
    createTableEstadual = (function(_this) {
      return function(graph) {
        var data, estado, rate, state, sum, year, _k, _l, _len, _len1, _ref14, _ref15, _results;
        data = [];
        sum = 0;
        years = H5.Data.years;
        year = years[chart12.options.period];
        _ref14 = ["terra_indigena", "floresta", "uc_integral", "uc_sustentavel"];
        _results = [];
        for (_k = 0, _len = _ref14.length; _k < _len; _k++) {
          rate = _ref14[_k];
          switch (rate) {
    ...
    createTableFederal = (function(_this) {
      return function(graph) {
        var data, estado, partialSum, rate, state, sum, sumUC, year, _k, _l, _len, _len1, _ref14, _ref15, _results;
        data = [];
        sum = 0;
        sumUC = 0;
        years = H5.Data.years;
        year = years[chart12.options.period];
        _ref14 = ["assentamento", "uc_integral_estadual", "uc_sustentavel_estadual", "dominio"];
        _results = [];
        for (_k = 0, _len = _ref14.length; _k < _len; _k++) {
          rate = _ref14[_k];
          switch (rate) {
    ...
    createTableCompetencias = (function(_this) {
      return function(graph) {
        var data, estado, rate, state, sumEstadual, sumFederal, territory, year, _k, _l, _len, _len1, _len2, _m, _ref14, _ref15, _ref16, _results;
        data = [];
        sumFederal = 0;
        sumEstadual = 0;
        years = H5.Data.years;
        year = years[chart12.options.period];
        _ref14 = H5.Data.statesProdes;
        for (_k = 0, _len = _ref14.length; _k < _len; _k++) {
          state = _ref14[_k];
          _ref15 = ['uc_sustentavel', 'uc_integral', 'terra_indigena', 'floresta', 'assentamento', 'dominio'];
          for (_l = 0, _len1 = _ref15.length; _l < _len1; _l++) {
            territory = _ref15[_l];
            switch (territory) {
              case 'uc_sustentavel':
    ...
    this.createDataTable();
    for (i = _k = 0, _ref14 = this.options.numberOfBox - 1; 0 <= _ref14 ? _k <= _ref14 : _k >= _ref14; i = 0 <= _ref14 ? ++_k : --_k) {
    ...
    _results = [];
    for (i = _l = 0, _ref15 = this.options.numberOfBox - 1; 0 <= _ref15 ? _l <= _ref15 : _l >= _ref15; i = 0 <= _ref15 ? ++_l : --_l) {
  ...
  chart13.drawChart = function() {
    var areaSelected, createTable, options, yearSelected, _k, _ref14;
    createTable = (function(_this) {
      return function(states) {
        var areaSelected, data, domainSelected, key, ranking, reg, yearSelected, _results;
        data = [];
        i = 1;
    ...
    this.createDataTable();
    for (i = _k = 0, _ref14 = this.options.numberOfBox - 1; 0 <= _ref14 ? _k <= _ref14 : _k >= _ref14; i = 0 <= _ref14 ? ++_k : --_k) {
      this.data[0].addColumn("number", "Ranking");
      this.data[0].addColumn("string", "Nome");
      this.data[0].addColumn("number", "Área");
    }
    createTable("nenhumEstado");
    yearSelected = charts_content._yearsSlctCrossingData.value;
  ...
  $(document).ready(function() {
    ...
    if (charts_content._shapesSlct.value === "terra_indigena" || charts_content._shapesSlct.value === "floresta") {
      charts_content._domainSlct.options["0"].selected = true;
      return charts_content._domainSlct.disabled = true;
    } else {
      return charts_content._domainSlct.disabled = false;
